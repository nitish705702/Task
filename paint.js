    ...
    canvas.addEventListener('dblclick', function (e) {
        ...
        dataPool.forEach(function (value) {
            ...
            if (Math.round(Area) === Math.round(Area1 + Area2 + Area3)) {
                ...
                clearCanvas();
                dataPool.forEach(function (value2) {
                    reDrawRactangles(value2[0][0], value2[0][1], value2[4], value2[3]);
                });
                return true;
            }
        });
...
//one mousedown listener
    canvas.addEventListener('mousedown', function (e) {
        ...
        if (dataPool.length > 0) {
            changeDistance.x = dataPool[reDrawRactangles][0][0] - mousePos.x;
            changeDistance.y = dataPool[reDrawRactangles][0][1] - mousePos.y
        }
...
//movement listener
    canvas.addEventListener('mousemove', function (e) {
        ...
        if (isActive && flag) {
            ...
            reDrawRactangles(startCoordinates.x, startCoordinates.y, calculateLineDistance(startCoordinates.x, startCoordinates.y, endCoordinates.x, endCoordinates.y), randomRGB);
            dataPool.forEach(function (value) {
                reDrawRactangles(value[0][0], value[0][1], value[4], value[3]);
            });
        } else if (isActive) {
            canvas.style.cursor = 'crosshair';
            clearCanvas();
            // doDragTranslationAtMove(endCoordinates.x, endCoordinates.y);
            var item = dataPool[reDrawRactangles];
            ...
            item[2][0] += difX;
            item[2][1] += difY;
            reDrawRactangles(item[0][0], item[0][1], item[4], item[3]);
            dataPool.forEach(function (value) {
...
//adding click listener for the clear button
    ...
    function checkIfInside(pos) {
        ...
        dataPool.forEach(function (value) {
            ...
            if (Math.round(Area) === Math.round(Area1 + Area2 + Area3)) {
                reDrawRactangles = dataPool.indexOf(value);
                flag = false;
                return true;
            }
        });
...
// function used to do translation
    function doDragTranslation(newx, newy) {
        ...
        item[2][0] += difX;
        item[2][1] += difY;
        dataPool.splice(reDrawRactangles, 0, item);
        clearCanvas();
        dataPool.forEach(function (value) {
            reDrawRactangles(value[0][0], value[0][1], value[4], value[3]);
        });
...
// function to get current mouse position
    ...
    function reDrawRactangles(x1, y1, distance, color) {
...
//        context.lineTo(x1 + distance / 2, y1 + height);
  //      context.lineTo(x1 - distance / 2, y1 + height);
        ...
        dataPool.push([[x1, y1], [x1 + x2-x1, y1 + y2-y1 ], [x1 - x2-x1, y1 + y2-y1 ],[x2,y2] [context.fillStyle], [distance]]);
